    ...
    <main className="booking-shell">
      <div className="booking-container">
        <header className="booking-header">
          <BrcLogo />
          <Button variant="secondary" onClick={() => window.history.back()}>Voltar ao sistema</Button>
        </header>
          ...
          <div className="booking-panel">
            <div className="tabs">
              {steps.map((item, index) => (
                <button key={item} className={index === step ? "tab active" : "tab"} onClick={() => setStep(index)}>{index + 1}. {item}</button>
              ))}
            </div>
            <div style={{ marginTop: 20 }}>
              {step === 0 && (
                <div className="card-list">
                  {services.slice(0, 8).map((item) => (
                    <button className={item.name === service ? "command-card payment-method active" : "command-card payment-method"} key={item.id} onClick={() => setService(item.name)}>
              ...
              {step === 1 && (
                <div className="card-list">
                  {professionals.slice(0, 6).map((item) => (
                    <button className={item.name === professional ? "command-card payment-method active" : "command-card payment-method"} key={item.id} onClick={() => setProfessional(item.name)}>
                      <strong>{item.name}</strong>
              ...
              {step === 2 && (
                <div className="mini-grid">
                  {["04/07 10:00", "04/07 14:00", "05/07 09:30", "05/07 16:00", "06/07 11:00", "06/07 18:00"].map((item) => (
                    <button className="payment-method" key={item}>{item}</button>
                  ))}
                </div>
              )}
              {step === 3 && (
              ...
              {step === 4 && (
                <SectionCard title="Seu horário foi reservado." eyebrow="Confirmação">
                  ...
                  <div className="notice">{service} com {professional}. Sinal e confirmação são apenas visuais neste beta.</div>
                </SectionCard>
              )}
            </div>
            <div className="button-row" style={{ marginTop: 20 }}>
              <Button variant="ghost" disabled={step === 0} onClick={() => setStep((current) => Math.max(0, current - 1))}>Voltar</Button>
              <Button icon={step === 4 ? <CheckCircle2 size={16} /> : <ArrowRight size={16} />} onClick={() => setStep((current) => Math.min(4, current + 1))}>
                {step === 4 ? "Confirmado" : "Continuar"}
              </Button>
            </div>
          </div>
        </section>
